...
import { parsePrivateKey, privateKeyToAddress, defaultDerivation, type Derivation } from '@/pearl/wallet';
...
export async function createWallet(args: {
  ...
  await remove(IMPORTED_KEY);   // fresh wallet — no imported keys
}
...
export async function clearWallet(): Promise<void> {
  await remove(WALLET_KEY, META_KEY, IMPORTED_KEY);
}
...
export async function importPrivateKeyAccount(args: {
  ...
  if (meta.accounts.some(a => a.type === 'imported' && a.address === address)) {
    throw new Error('That private key is already imported.');
  }
