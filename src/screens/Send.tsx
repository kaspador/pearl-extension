  ...
  return (
    <div className="flex-1 flex flex-col">
      <header className="flex items-center gap-3 px-5 pt-4 pb-3 border-b border-ink-800">
        <button onClick={() => step === 'form' ? onBack() : setStep('form')} className="text-pearl-500 hover:text-pearl-200 text-xs">←</button>
      ...
      {step === 'form' && (
        <div className="p-5 flex flex-col gap-4 flex-1">
          ...
          <label className="block">
            <span className="text-[10px] uppercase tracking-wider text-pearl-600 flex justify-between">
              <span>Amount</span>
              <button
          ...
          <div>
            <div className="text-[10px] uppercase tracking-wider text-pearl-600 mb-1.5">Fee</div>
            ...
            {c.feeRate != null && (
              <div className="text-[10px] text-pearl-700 mt-1">≈ {String(feeRate)} grains/vB</div>
            )}
          </div>
      ...
      {step === 'review' && (
        <div className="p-5 flex flex-col gap-4 flex-1">
          <div className="bg-ink-900 border border-ink-800 rounded-xl p-4 space-y-2.5 text-xs">
      ...
      {step === 'broadcast' && (
        <div className="p-5 flex-1 flex items-center justify-center text-xs text-pearl-500">
          Broadcasting…
        </div>
      )}
    </div>
...
function Row({ label, value, mono }: { label: string; value: string; mono?: boolean }) {
  return (
    <div className="flex items-start justify-between gap-3">
      <span className="text-pearl-600 text-[10px] uppercase tracking-wider shrink-0 pt-0.5">{label}</span>
